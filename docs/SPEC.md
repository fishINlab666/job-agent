...
>
> **这里刻意不写用例数** —— 它只写在 `README.md` 现状那一节，和产生它的命令、日期
> 放在一起。理由和这一行原先写错了什么，见文末变更记录 `2026-08-13` 那条。
> `tests/test_docs_match_code.py::TestTestCountHasOneHome` 守着这条约定。
...
CLI 全部命令（**12 条**）：`init` / `source-add` / `sync` / `refresh-grad-year` /
`repair-apply-url` / `jobs` / `digest` / `status` / `apply` / `applications` /
`checkup` / `health`。

> 这张清单漏过**三次**：`refresh-grad-year`（007 起）、`repair-apply-url`（010 起）、
> `source-add` / `checkup` / `health`（013–014 起）。前两次这里就写了核对办法
> 「拿代码当准：`uv run python -m jobagent.cli --help` 的输出才是全集」——
> 然后第三次照样漏了，因为那是一句**给人看的话，没人执行**。
> 现在 `tests/test_docs_match_code.py::TestSpecCliList` 把它变成了一条会红的测试。
...
6 个工具，全部只读：`list_jobs` / `explain_match` / `list_sources` /
...
- **`job_changes` 只交出采集侧事件。** 判据是**发射点级**：`ingest.py` 发的都给
  ...
  而不是靠一句注释。
- **`check_form_selectors` 是唯一对外发请求的工具**（起真浏览器访问对方招聘站，
  一次几十秒），其余五个只读本地库。
- 工具名一律 `list_` / `explain_` / `check_` 打头：动词打头才看得出它是读还是写。
...
新功能的验收 = 这个数**变大**且全绿。当前值看 `README.md` 现状那一节
（那里数字、日期、产生它的命令写在一起），或者直接跑上面这条看最后一行。

这一节以前在这里钉过一个基线数，现在不钉了 —— 理由见文末变更记录 `2026-08-13`
那条。**「变大且全绿」这个判据不需要知道当前值**：它比较的是同一条命令在改动前后的两次
输出，而写死的基线只会在下一次加测试时变成谣言。这一段本身不复述那个旧数字，因为
`tests/test_docs_match_code.py` 的守卫分不清「历史引用」和「当前声明」，它只会看见一个数。
...
| 2026-08-13 | **补记 013–019 七轮，并把「当前用例数」从这份文档里拆掉。**<br><br>①**§2 结构改了**：模块表加 M8（只读巡检：`checkup` 判据体检 + `health` 死链抽查）和 MCP 只读层两行，并给每行补「实现」列指到具体模块 —— 这份文档此前**一次都没引用过** `jobagent/*.py`，于是 `health.py` / `mcp_server.py` / `queries.py` 三个模块它完全不知道。新增 MCP 只读层一节（6 个工具、代投一步不许过来、`job_changes` 的发射点级判据）。<br><br>②**CLI 清单 9 → 12 条**，补 `source-add` / `checkup` / `health`。这张清单**漏第三次**了（前两次是 `refresh-grad-year`、`repair-apply-url`，都记在 §2）。前两次这里写下的核对办法「拿代码当准，`cli --help` 才是全集」是**一句给人看的话，没人执行**，所以第三次照样漏。现在改成一条会红的测试 `tests/test_docs_match_code.py::TestSpecCliList`，双向比集合。<br><br>③**页首和 §9 不再钉用例数。** 原先页首写「测试基线 → 464 passed」—— 发布 `0.2.0` 的 commit（`9d6ffb1`）上真实收集数是 **504**（用 `git worktree` 把那个 commit 拉出来 `--collect-only` 核过），也就是说 README 写 504、这里写 464，**同一个 commit 上两个数打对台，而 464 那个是错的**。`CLAUDE.md` 早写了「用例数不写死」这条规则，但它当时只管住了 `CLAUDE.md` 自己。现在数字只存在于 `README.md` 现状那一节（数字 + 日期 + 命令挨在一起），`TestTestCountHasOneHome` 守着。§9 的「变大且全绿」这个判据本来就不需要知道当前值 —— 它比的是同一条命令改动前后两次输出。<br><br>④**013–019 各自做了什么**：013 `apply_url` 周期性健康巡检（防 8594 条死链再次沉默失效）；014 MCP 只读层；015 域词抢走职能词（工程岗被判成设计岗）；016 diff 为空时不发 `job_updated`；017 `顾问`/`采购` 归族 + 判不出族的计数单位从标题条数改成**岗位型**；018 补 30 个职能词 + 换掉「加在表末尾改判 0」这个恒为 0 的假判据；019 `job_changes` 白名单补齐采集侧 3 种事件（库里丢着 8 条）+ 让「排除了什么」进返回值。逐轮的现场记录在 `docs/plans/013..019`，**不再往 WIKI 那张表里补** —— 理由见 `plans/020` §8。| `plans/013`–`plans/019` 均已复盘；`plans/020` |
