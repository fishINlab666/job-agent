...
TITLE_RULES: list[tuple[tuple[str, ...], str]] = [
    ...
    # `顾问` 是面客销售/咨询岗。加它救回 233 行 / 21 个岗位型（2026-08-13 快照）。
    # 【原本这里写着「加进这一组和加在表末尾，改判数都是 0 —— 说明它和别的族没有
    # 交叠」，后半句是错的，别照抄】「加在末尾改判 0」对任何词都成立，推不出「没有
    # 交叠」。为什么见下面 `采购` 那段的同名警告。`顾问` 放在这一组本身没问题 ——
    # 支持它的是**库级 0 改族**（scripts/measure_family_gaps.py db-effect），不是改判 0。
